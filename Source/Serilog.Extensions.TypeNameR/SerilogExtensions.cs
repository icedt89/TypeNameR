...
public static class SerilogExtensions
{
    ...
    public static ILogger For<T>(this ILogger logger, T instance, ITypeNameR? typeNameR = null, bool fullTypeName = true, NameRControlFlags nameRControlFlags = NameRControlFlags.All, bool throwExceptions = false,
        bool selfLog = false)
        where T : notnull
    {
        ...
        return logger.ForTypeCore(instance.GetType(), typeNameR ?? StaticTypeNameR.Instance, fullTypeName, nameRControlFlags, throwExceptions, selfLog);
    }
    ...
    public static ILogger For(this ILogger logger, Type type, ITypeNameR? typeNameR = null, bool fullTypeName = true, NameRControlFlags nameRControlFlags = NameRControlFlags.All, bool throwExceptions = false,
        bool selfLog = false)
    {
        ArgumentNullException.ThrowIfNull(logger);
        ArgumentNullException.ThrowIfNull(type);

        return logger.ForTypeCore(type, typeNameR ?? StaticTypeNameR.Instance, fullTypeName, nameRControlFlags, throwExceptions, selfLog);
    }

    private static ILogger ForTypeCore(this ILogger logger, Type type, ITypeNameR typeNameR, bool fullTypeName, NameRControlFlags nameRControlFlags, bool throwExceptions, bool selfLog)
    {
        try
        {
            var typeDisplay = typeNameR.GenerateDisplay(type, fullTypeName, nameRControlFlags);
    ...
    public static LoggerConfiguration WithExceptionDisplay(this LoggerEnrichmentConfiguration configuration,
        ITypeNameR? typeNameR = null,
        NameRControlFlags nameRControlFlags = NameRControlFlags.All,
        string exceptionDisplayPropertyName = ExceptionDisplayEnricher.DefaultExceptionDisplayPropertyName)
    {
        ArgumentNullException.ThrowIfNull(configuration);

        var exceptionDisplayEnricher = new ExceptionDisplayEnricher(typeNameR ?? StaticTypeNameR.Instance, nameRControlFlags, exceptionDisplayPropertyName);
